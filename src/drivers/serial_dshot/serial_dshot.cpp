...
void SerialDShot::process_rx_frame(const uint8_t *frame, size_t length)
{
	...
	for (unsigned i = 0; i < DSHOT_CHANNELS; ++i) {
		...
		// Adapter failures are encoded as 0xff00 | error_code. 0xff80 is the
		// protocol's valid zero-motion value (12-bit telemetry payload 0xfff).
		if ((period != 0 && period < 0xff00) || period == 0xff80) {
...
void SerialDShot::publish_esc_status()
{
	...
	for (unsigned channel = 0; channel < DSHOT_CHANNELS; ++channel) {
		...
		if (_telemetry_valid_mask & (1u << channel)) {
			...
			esc.esc_rpm = _last_period[channel] == 0xff80 ? 0 : static_cast<int32_t>(120000000ULL /
...
int SerialDShot::print_status()
{
	...
	if (_last_telemetry_time != 0) {
		...
		PX4_INFO("telemetry errors: %u %u %u %u", _telemetry_errors[0], _telemetry_errors[1],
			 _telemetry_errors[2], _telemetry_errors[3]);
	}
